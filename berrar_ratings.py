...
if __name__ == '__main__':
    ...
    for file in ["trainset_22-23_exact_44"]:
        ...
        for league in df.Lge.unique():
            ...
            df1.to_csv(f"berrar rating/{file}_{league}.csv", index=False)
            with open(f'berrar rating/{file}_{league}_berrarratings_hyperparameters.pickle', 'wb') as handle:
                pickle.dump(pos, handle, protocol=pickle.HIGHEST_PROTOCOL)
            with open(f'berrar rating/{file}_{league}_team_ratings_dict.pickle', 'wb') as handle:
